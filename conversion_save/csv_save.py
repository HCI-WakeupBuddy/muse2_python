...
def analyze_eeg(data, sampling_rate):
    ...
    gamma_band = np.where((freqs >= 31) & (freqs <= 50))[0]
...
def main():
    ...
    sampling_rate = 256
    fft_magnitudes, freqs, delta_band, theta_band, alpha_band, beta_band, gamma_band = analyze_eeg(data, sampling_rate)

    print("Visualizing EEG data...")
    visualize_data(fft_magnitudes, freqs, [delta_band, theta_band, alpha_band, beta_band, gamma_band], 
